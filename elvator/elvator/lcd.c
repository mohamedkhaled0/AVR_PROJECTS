...
void lcd_send(uint8_t value, uint8_t mode) {
	if (mode) {
		LCD_CONTROL_R |=  (1 << LCD_RS);
		} else {
		LCD_CONTROL_R = LCD_CONTROL_R & ~(1 << LCD_RS);
	}

	LCD_CONTROL_R = LCD_CONTROL_R /*& ~(1 << LCD_RW)*/;
...
void lcd_write_nibble(uint8_t nibble) {
	LCD_DATA_R = (LCD_DATA_R & 0xff & ~(0x0f << LCD_D0)) | ((nibble & 0x0f) << LCD_D0);

	LCD_CONTROL_R = LCD_CONTROL_R & ~(1 << LCD_EN);
	LCD_CONTROL_R = LCD_CONTROL_R | (1 << LCD_EN);
	LCD_CONTROL_R = LCD_CONTROL_R & ~(1 << LCD_EN);
